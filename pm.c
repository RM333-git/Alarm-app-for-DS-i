 ...
 * This file has been modified by miri on 2026/05/30.
 * Changes made: Modified pmEnterSleep for the DS(i) alarm clock application.
 */
...
void pmEnterSleep(void)
{
...
#if defined(ARM9)
	// In DSi mode (TWL), turn off the power to peripheral devices.
	if (systemIsTwlMode()) {
	...
	// Read the PMIC control register (register 0).
	// Specify 0 or PmicReg_Control if the name is not found.
	unsigned pmic_ctl = pmicReadRegister(PmicReg_Control);

	// Turn off the backlight (Bit 2, 3) and sound (Bit 0, 1).
	unsigned pmic_sleep_val = pmic_ctl & ~(PMIC_CTRL_LCD_BL_TOP | PMIC_CTRL_LCD_BL_BOTTOM | PMIC_CTRL_SOUND_MUTE | PMIC_CTRL_SOUND_ENABLE);
	// Change the LED setting to "slow blinking" (while maintaining other bits).
	pmic_sleep_val |= PMIC_CTRL_LED_BLINK_SLOW;

	// Write register
	pmicWriteRegister(PmicReg_Control, pmic_sleep_val);

	// Turn off WiFi (Mitsumi) (using REG_POWCNT)
	u16 old_powcnt = REG_POWCNT;
	if (old_powcnt & POWCNT_WL_MITSUMI) {
		REG_POWCNT = old_powcnt & ~POWCNT_WL_MITSUMI;
	}
	
	u32 ie = REG_IE;
	u32 ie2 = REG_IE2;
	REG_IE = IRQ_HINGE | IRQ_RTC;
	REG_IE2 = 0;

	// (This lines are originally here)
	// unsigned pmic_ctl = pmicReadRegister(PmicReg_Control);
	...
	// After waking from sleep mode, restore power to peripheral devices.
	if (systemIsTwlMode()) {
		scfgSetMcPower(true);
	}
